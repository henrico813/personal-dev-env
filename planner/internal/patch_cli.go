...
func runPatch(args []string, stdout, stderr io.Writer) int {
	...
	if err != nil {
		reportError(stderr, "patch", newPlannerCLIError(PlannerDecodeInputError, err, "structured patch"))
		return 1
	}
...
func parsePlannerPatch(raw string) ([]patchOp, error) {
	...
	for i := 1; i < len(lines); {
		...
		if strings.TrimSpace(line) == "*** End Patch" {
			...
			if len(ops) == 0 {
				return nil, fmt.Errorf("patch must include at least one operation")
			}
			return ops, nil
		}
		if line == "" {
			return nil, fmt.Errorf("expected patch header, got empty line")
		}
		if !strings.HasPrefix(line, "*** ") {
			return nil, fmt.Errorf("expected patch header, got %q", line)
		}
...
func parsePlannerPatchOp(lines []string, start int) (patchOp, int, error) {
	...
	if !ok {
		return patchOp{}, 0, fmt.Errorf("malformed patch header %q", lines[start])
	}
	...
	default:
		return patchOp{}, 0, fmt.Errorf("unknown patch header %q", lines[start])
	}
	...
	case patchOpUpdateField:
		...
		for _, line := range bodyLines {
			if line == "" {
				return patchOp{}, 0, fmt.Errorf("update field bodies must prefix every line with + or -")
			}
			switch line[0] {
			case '-':
		...
		if len(oldLines) == 0 || len(newLines) == 0 {
			return patchOp{}, 0, fmt.Errorf("update field bodies require both - and + lines")
		}
		op.OldText = strings.Join(oldLines, "\n")
		op.NewText = strings.Join(newLines, "\n")
	case patchOpAddItem:
		if len(bodyLines) != 1 {
			return patchOp{}, 0, fmt.Errorf("checklist edits must be single-line")
		}
		line := bodyLines[0]
		if line == "" || line[0] != '+' {
			return patchOp{}, 0, fmt.Errorf("checklist edits must be single-line")
...
func applyPlannerPatchField(plan *Plan, selector, oldText, newText string) error {
	...
	case "title":
		if plan.Title != oldText {
			return fmt.Errorf("patch old value mismatch for title")
		}
		plan.Title = newText
	case "overview":
		if plan.Overview != oldText {
			return fmt.Errorf("patch old value mismatch for overview")
		}
		plan.Overview = newText
	case "definition_of_done.narrative":
		if plan.DefinitionOfDone.Narrative != oldText {
			return fmt.Errorf("patch old value mismatch for definition_of_done.narrative")
		}
	...
	case "definition_of_done.current_state":
		if plan.DefinitionOfDone.CurrentState != oldText {
			return fmt.Errorf("patch old value mismatch for definition_of_done.current_state")
		}
	...
	case "definition_of_done.module_shape":
		if plan.DefinitionOfDone.ModuleShape != oldText {
			return fmt.Errorf("patch old value mismatch for definition_of_done.module_shape")
		}
	...
	case "verification.summary":
		...
		if plan.Verification.Summary != oldText {
			return fmt.Errorf("patch old value mismatch for verification.summary")
		}
		plan.Verification.Summary = newText
	default:
		return fmt.Errorf("unsupported patch selector %q", selector)
	}
	return nil
}
