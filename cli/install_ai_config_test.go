...
func TestAIConfigPreservesSkillBoundaries(t *testing.T) {
	...
	if len(migrated) != 1 {
		t.Fatalf("migrated backups = %v", migrated)
	}

	if err := os.RemoveAll(filepath.Join(cfg.AIRepoDir, "skills", "self-improvement")); err != nil {
		t.Fatal(err)
	}
	if err := syncAIConfig(cfg, Runner{}); err != nil {
	...
	for _, root := range []string{filepath.Join(cfg.HomeDir, ".agents", "skills"), filepath.Join(cfg.CodexConfigDir, "skills")} {
		if _, err := os.Stat(filepath.Join(root, "self-improvement")); !os.IsNotExist(err) {
			t.Fatalf("stale skill remains: %v", err)
		}
	...
	if err != nil || len(backups) != 1 {
		t.Fatalf("stale backups = %v, err = %v", backups, err)
	}
	mustFileContents(t, agents, "personal agents\n")
	mustFileContents(t, codex, "personal codex\n")
	mustLinkTarget(t, filepath.Join(cfg.CodexConfigDir, "skills", "create-plan", "bin", "planner"), planner)

	if err := os.RemoveAll(filepath.Join(cfg.AIRepoDir, "codex", "skills", "create-plan")); err != nil {
		t.Fatal(err)
	}
	if err := syncAIConfig(cfg, Runner{}); err != nil {
		t.Fatalf("remove Codex skill: %v", err)
	}
	if _, err := os.Stat(filepath.Join(cfg.CodexConfigDir, "skills", "create-plan")); !os.IsNotExist(err) {
		t.Fatalf("stale Codex skill remains: %v", err)
	}
}
...
func TestAIConfigRejectsSkillCollisions(t *testing.T) {
	for _, root := range []func(*Config) string{
		func(c *Config) string {
			return filepath.Join(c.HomeDir, ".agents", "skills", "self-improvement", "SKILL.md")
		},
		func(c *Config) string {
			return filepath.Join(c.CodexConfigDir, "skills", "self-improvement", "SKILL.md")
		},
	} {
		cfg := newAIConfigFixture(t)
		...
		if err == nil || !strings.Contains(err.Error(), "unmanaged skill collision") {
			t.Fatalf("collision error = %v", err)
		}
		mustFileContents(t, root(cfg), "user skill\n")
		if _, err := os.Stat(filepath.Join(cfg.AIRuntimeDir, "ai-config-backups")); !os.IsNotExist(err) {
...
func TestAIConfigValidatesSkillMetadata(t *testing.T) {
	...
	for i, content := range contents {
		...
		if i == len(contents)-1 {
			...
			mustWriteFile(t, target, content, 0o644)
			os.Remove(path)
		...
		if err := syncAIConfig(cfg, Runner{}); err == nil {
			t.Fatalf("case %d: expected metadata error", i)
		}
	}
}
...
func TestAIConfigRejectsOwnershipState(t *testing.T) {
	for _, data := range []string{"{", `{"version":2}`, `{"version":1,"shared":["git-messages","git-messages"]}`, `{"version":1,"codex":["BadName"]}`} {
		...
		if _, err := os.Stat(filepath.Join(cfg.AIRuntimeDir, "ai-config-backups")); !os.IsNotExist(err) {
			t.Fatalf("created backups: %v", err)
		}
	}
}
...
func TestAIConfigPreflightsEverySource(t *testing.T) {
	...
	for i, path := range paths {
		for _, dryRun := range []bool{false, true} {
			cfg := newAIConfigFixture(t)
			mustWriteFile(t, filepath.Join(cfg.OpenCodeConfigDir, "AGENTS.md"), "old config\n", 0o644)
			if err := os.RemoveAll(path(cfg)); err != nil {
				t.Fatal(err)
			}
			if err := syncAIConfig(cfg, Runner{DryRun: dryRun}); err == nil {
				t.Fatalf("case %d dry=%t: expected source error", i, dryRun)
			}
			mustFileContents(t, filepath.Join(cfg.OpenCodeConfigDir, "AGENTS.md"), "old config\n")
			if _, err := os.Stat(filepath.Join(cfg.AIRuntimeDir, "ai-config-backups")); !os.IsNotExist(err) {
				t.Fatalf("case %d created backups", i)
			}
		}
	}
}
...
func TestAIConfigRollsBackActivation(t *testing.T) {
	root := t.TempDir()
	first := filepath.Join(root, "live", "first")
	second := filepath.Join(root, "live", "second")
	stage := filepath.Join(root, "stage", "first")
	mustWriteFile(t, first, "old first\n", 0o644)
	mustWriteFile(t, second, "old second\n", 0o644)
	mustWriteFile(t, stage, "new first\n", 0o644)
	...
	if _, err := activateAIConfigChanges(changes); err == nil {
...
func TestAIConfigRejectsGeneratedPath(t *testing.T) {
	cfg := newAIConfigFixture(t)
	outside := filepath.Join(t.TempDir(), "outside")
	if err := os.MkdirAll(outside, 0o755); err != nil {
		t.Fatal(err)
	}
	bin := filepath.Join(cfg.AIRepoDir, "codex", "skills", "create-plan", "bin")
	if err := os.Symlink(outside, bin); err != nil {
		t.Fatal(err)
	}
	mustWriteFile(t, filepath.Join(cfg.AIRuntimeDir, "planner", "planner"), "planner\n", 0o755)
	if err := syncAIConfig(cfg, Runner{}); err == nil {
		t.Fatal("expected generated path error")
	}
	if _, err := os.Stat(filepath.Join(outside, "planner")); err == nil {
		t.Fatal("launcher escaped stage")
	}
}

func TestAIConfigDryRunChecksPlanner(t *testing.T) {
	cfg := newAIConfigFixture(t)
	if err := os.MkdirAll(filepath.Join(cfg.AIRuntimeDir, "planner", "planner"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := syncAIConfig(cfg, Runner{DryRun: true}); err == nil {
		t.Fatal("expected planner validation error")
	}
}

func TestAIConfigMovesAcrossFilesystems(t *testing.T) {
	if info, err := os.Stat("/dev/shm"); err != nil || info.Sys() == nil {
		t.Skip("no shared memory filesystem")
	}
	source := filepath.Join(t.TempDir(), "source")
	destination := filepath.Join("/dev/shm", "pde-ai-config-test-"+filepath.Base(t.TempDir()))
	mustWriteFile(t, source, "source\n", 0o644)
	defer os.RemoveAll(destination)
	if err := moveAIConfigEntry(source, destination); err != nil {
		t.Fatalf("cross-filesystem move: %v", err)
	}
	mustFileContents(t, destination, "source\n")
	if _, err := os.Stat(source); !os.IsNotExist(err) {
		t.Fatalf("source remains: %v", err)
	}
}

func TestAIConfigRefusesConcurrentRestore(t *testing.T) {
	root := t.TempDir()
	destination := filepath.Join(root, "destination")
	backup := filepath.Join(root, "backup")
	mustWriteFile(t, destination, "installed\n", 0o644)
	active, err := os.Lstat(destination)
	if err != nil {
		t.Fatal(err)
	}
	mustWriteFile(t, backup, "previous\n", 0o644)
	if err := os.Rename(destination, filepath.Join(root, "active")); err != nil {
		t.Fatal(err)
	}
	mustWriteFile(t, destination, "concurrent\n", 0o644)
	if err := restoreAIConfigChange(aiConfigChange{destination: destination, backupPath: backup}, true, active); err == nil {
		t.Fatal("expected concurrent restore error")
	}
	mustFileContents(t, destination, "concurrent\n")
}
...
func TestAIConfigCommandUsesDryRun(t *testing.T) {
	...
	home := t.TempDir()
	t.Setenv("HOME", home)
	writeAIConfigSources(t, repo)
	mustWriteFile(t, filepath.Join(repo, "pde", "config", ".keep"), "", 0o644)
	...
	if !strings.Contains(out.String(), "DRY-RUN: stage AI config") {
		t.Fatalf("missing actions: %s", out.String())
	}
	if _, err := os.Stat(filepath.Join(home, ".local", "share", "pde", "ai")); !os.IsNotExist(err) {
		t.Fatalf("dry-run changed runtime: %v", err)
	}
}

func TestAIConfigHelpListsTarget(t *testing.T) {
	out, _, err := executeInstallCmd(t, "install", "--help")
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(out.String(), "ai-config") {
		t.Fatal("help missing target")
	}
}
func TestAIConfigErrorsListTarget(t *testing.T) {
	repo := t.TempDir()
	t.Setenv("HOME", t.TempDir())
	mustWriteFile(t, filepath.Join(repo, "pde", "config", ".keep"), "", 0o644)
	_, _, err := executeInstallCmd(t, "install", "missing", "--repo-root", repo)
	if err == nil || !strings.Contains(err.Error(), "ai-config") {
...
func newAIConfigFixture(t *testing.T) *Config {
	t.Helper()
	root := t.TempDir()
	cfg := &Config{RepoRoot: root, HomeDir: filepath.Join(root, "home"), AIRepoDir: filepath.Join(root, "ai"), AIRuntimeDir: filepath.Join(root, "home", ".local", "share", "pde", "ai"), OpenCodeConfigDir: filepath.Join(root, "home", ".config", "opencode"), CodexConfigDir: filepath.Join(root, "home", ".codex"), PiAgentDir: filepath.Join(root, "home", ".pi", "agent")}
	writeAIConfigSources(t, root)
	return cfg
}
func writeAIConfigSources(t *testing.T, root string) {
	t.Helper()
	mustWriteFile(t, filepath.Join(root, "ai", "AGENTS.md"), "agents\n", 0o644)
	mustWriteFile(t, filepath.Join(root, "ai", "opencode", "agents", "review.md"), "agent\n", 0o644)
	mustWriteFile(t, filepath.Join(root, "ai", "opencode", "commands", "review.md"), "command\n", 0o644)
	mustWriteFile(t, filepath.Join(root, "ai", "pi", "agent", "settings.json"), "{}\n", 0o644)
	for _, pair := range []struct{ root, name string }{{filepath.Join(root, "ai", "codex", "skills"), "create-plan"}, {filepath.Join(root, "ai", "skills"), "git-messages"}, {filepath.Join(root, "ai", "skills"), "self-improvement"}} {
		writeSkillFixture(t, pair.root, pair.name)
	}
