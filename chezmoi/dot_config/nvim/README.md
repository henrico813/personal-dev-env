...
SHA-256 checksum to the `full` branch of
...
The maintained `henrico813/codecompanion.nvim` fork has no compatible release, so chezmoi pins the resolved `main` commit and archive checksum instead of following the branch.
...
Inline editing is configured through a local OpenAI-compatible shim named `opencode-inline-shim`. Install it and the pinned `opencode`, `surveil`, and `vibe` tools with `pde-installer install`.
...
CodeCompanion depends on `plenary.nvim`; chezmoi installs both pinned plugin archives. Verify `which opencode` and `which opencode-inline-shim` both return paths before opening chat or inline.
