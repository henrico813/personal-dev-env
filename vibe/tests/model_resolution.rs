...
fn build_image() {
    ...
    let output = Command::new("docker")
        .args(["build", "-t", "vibe-pi:0.5.0", "-f", "docker/Dockerfile", "."])
        .current_dir(root)
        .output()
        .expect("docker build");
    assert!(output.status.success(), "docker build failed: {}", String::from_utf8_lossy(&output.stderr));
}
...
fn run_json_persists_requested_and_resolved_selectors() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_vibe"))
        .args([
            "run", "--key", "fixture-model-resolution", "--prompt-file",
            prompt.path().to_str().expect("prompt path"), "--model", "dynamic-model",
        ])
        .env("HOME", home.path())
        .output()
        .expect("run vibe run");
