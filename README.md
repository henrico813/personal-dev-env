...
| OpenCode | `ai/opencode/` | `~/.config/opencode/{agents,commands}`, `opencode.json` permission merge | OpenCode commands and agents |
...
The AI installer backs up the managed OpenCode, Codex, and Pi paths before replacement. It also merges an XDG-aware `permission.external_directory` allowance for Surveil state into user-owned `opencode.json`, backing up that file only when the merge changes it. Other root settings remain in place.
