...
func TestColorDataHasCompletePalettes(t *testing.T) {
	...
	for _, name := range wantProfiles {
		...
		if !ok { t.Errorf("colorProfiles[%q] is missing", name); continue }
		terminal, ok := profileData["terminal"].(map[string]any)
		if !ok { t.Errorf("colorProfiles[%q].terminal is missing", name); continue }
		...
		for _, set := range []string{"normal", "bright"} {
			...
			if !ok { t.Errorf("colorProfiles[%q].terminal.%s is missing", name, set); continue }
			assertColorFields(t, hexColor, palette, "black", "red", "green", "yellow", "blue", "magenta", "cyan", "white")
			if len(palette) != 8 { t.Errorf("colorProfiles[%q].terminal.%s has %d colors, want 8", name, set, len(palette)) }
		}
		ui, ok := profileData["ui"].(map[string]any)
		if !ok { t.Errorf("colorProfiles[%q].ui is missing", name) } else { assertColorFields(t, hexColor, ui, "surface", "muted", "subtle", "accent", "secondary", "success", "error", "warning") }
		for _, field := range []string{"label", "bat"} { if value, ok := profileData[field].(string); !ok || value == "" { t.Errorf("colorProfiles[%q].%s is missing", name, field) } }
		...
		if !ok || !colorschemeOK || colorscheme == "" || !lualineOK || lualine == "" { t.Errorf("colorProfiles[%q].nvim is incomplete", name) }
	}
}

func TestColorTemplatesRenderEveryProfile(t *testing.T) {
	templates := []string{"dot_config/alacritty/alacritty.toml.tmpl", "dot_config/wezterm/wezterm.lua.tmpl", "dot_tmux.conf.tmpl", "dot_p10k.zsh.tmpl", "dot_zshrc.tmpl", "dot_config/nvim/lua/plugins/colorscheme.lua.tmpl", "dot_config/nvim/lua/plugins/ui.lua.tmpl"}
	for _, name := range templates { t.Run(name, func(t *testing.T) { outputs := make(map[[32]byte]string); for _, profile := range colorprofile.All() { text, err := renderConfiguredTemplate(t, name, "full", string(profile)); if err != nil { t.Fatalf("render %s: %v", profile, err) }; if strings.Contains(text, "{{") { t.Fatalf("render %s left a template action", profile) }; outputs[sha256.Sum256([]byte(text))] = string(profile) }; if len(outputs) != len(colorprofile.All()) { t.Fatalf("rendered %d distinct outputs, want %d", len(outputs), len(colorprofile.All())) } }) }
}

func TestPowerlevelUsesRGBColors(t *testing.T) {
	for _, profile := range colorprofile.All() { text, err := renderConfiguredTemplate(t, "dot_p10k.zsh.tmpl", "full", string(profile)); if err != nil { t.Fatal(err) }; path := filepath.Join(t.TempDir(), ".p10k.zsh"); if err := os.WriteFile(path, []byte(text), 0o600); err != nil { t.Fatal(err) }; command := exec.Command("zsh", "-c", `source "$1"
...
done`, "zsh", path); if output, err := command.CombinedOutput(); err != nil { t.Fatalf("%s leaves a non-RGB prompt color: %v\n%s", profile, err, output) } }
}

func TestRenderedConfigsAvoidPaletteChanges(t *testing.T) {
	for _, name := range []string{"dot_tmux.conf.tmpl", "dot_p10k.zsh.tmpl", "dot_zshrc.tmpl"} { text, err := renderConfiguredTemplate(t, name, "full", "everforest-dark"); if err != nil { t.Fatal(err) }; for _, escape := range []string{"]4;", "]10;", "]11;", "]12;"} { if strings.Contains(text, escape) { t.Errorf("%s emits terminal palette selector %q", name, escape) } } }
}

func TestWezTermRendersInterfaceColors(t *testing.T) {
	text, err := renderConfiguredTemplate(t, "dot_config/wezterm/wezterm.lua.tmpl", "full", "everforest-dark"); if err != nil { t.Fatal(err) }; for _, field := range []string{"scrollbar_thumb", "split"} { if !strings.Contains(text, field+" = '#") { t.Errorf("rendered WezTerm config omits %s", field) } }
}

func assertColorFields(t *testing.T, pattern *regexp.Regexp, values map[string]any, fields ...string) { t.Helper(); for _, field := range fields { value, ok := values[field].(string); if !ok || !pattern.MatchString(value) { t.Errorf("%s = %v, want #RRGGBB", field, values[field]) } } }
...
func loadTemplateData(t *testing.T) map[string]any {
	...
	if err := json.Unmarshal(data, &values); err != nil { t.Fatal(err) }
	return values
}
