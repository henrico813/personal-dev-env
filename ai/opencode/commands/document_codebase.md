---
description: Diagnose documentation problems and fix them at the right level
---
...
5. In review mode, delegate one bounded pass to the read-only `docs-reviewer`,
   report its supported findings with file and line references, and stop before
   editing. Do not delegate review mode to `docs-writer`.
...
For bounded edit work, edit directly. For an explicitly broad edit, the
`docs-reviewer` and `docs-writer` agents are optional. Pass an `Applicable
