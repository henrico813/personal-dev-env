...
- For exact filename lookups, start with `pde vault locate --json --vault default` and widen to `--vault any` only after a miss. Treat `ambiguous`, `error`, and a final `not_found` as stop-and-ask states.
