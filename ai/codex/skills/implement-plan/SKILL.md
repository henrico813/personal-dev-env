---
name: implement-plan
description: Use when the user asks to implement an approved design or implementation plan with verification.
---
...
then other configured providers (excluding `opencode-go`). If model resolution
returns `setup_error`, diagnose it with `vibe resolve-model --model "$MODEL"`
(and the same `--provider` when one was requested).
