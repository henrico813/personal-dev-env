...
ORIGINAL_EVAL_BASE=${EVAL_BASE-}
ORIGINAL_EVAL_BASE_SET=${EVAL_BASE+x}
ORIGINAL_EVAL_CASES=${EVAL_CASES-}
ORIGINAL_EVAL_CASES_SET=${EVAL_CASES+x}
ORIGINAL_EVAL_TRACES=${EVAL_TRACES-}
ORIGINAL_EVAL_TRACES_SET=${EVAL_TRACES+x}
ORIGINAL_INSTALLER=${INSTALLER-}
ORIGINAL_INSTALLER_SET=${INSTALLER+x}
ORIGINAL_EVAL_PROMPT=${EVAL_PROMPT-}
ORIGINAL_EVAL_PROMPT_SET=${EVAL_PROMPT+x}
ORIGINAL_MODEL_FAILURE=${MODEL_FAILURE-}
ORIGINAL_MODEL_FAILURE_SET=${MODEL_FAILURE+x}
ORIGINAL_PDE_REPO_ROOT=${PDE_REPO_ROOT-}
ORIGINAL_PDE_REPO_ROOT_SET=${PDE_REPO_ROOT+x}
ORIGINAL_PYTHONDONTWRITEBYTECODE=${PYTHONDONTWRITEBYTECODE-}
ORIGINAL_PYTHONDONTWRITEBYTECODE_SET=${PYTHONDONTWRITEBYTECODE+x}
...
restore_environment() {
  ...
  if [[ -n "$ORIGINAL_EVAL_BASE_SET" ]]; then export EVAL_BASE="$ORIGINAL_EVAL_BASE"; else unset EVAL_BASE; fi
  if [[ -n "$ORIGINAL_EVAL_CASES_SET" ]]; then export EVAL_CASES="$ORIGINAL_EVAL_CASES"; else unset EVAL_CASES; fi
  if [[ -n "$ORIGINAL_EVAL_TRACES_SET" ]]; then export EVAL_TRACES="$ORIGINAL_EVAL_TRACES"; else unset EVAL_TRACES; fi
  if [[ -n "$ORIGINAL_INSTALLER_SET" ]]; then INSTALLER="$ORIGINAL_INSTALLER"; else unset INSTALLER; fi
  if [[ -n "$ORIGINAL_EVAL_PROMPT_SET" ]]; then export EVAL_PROMPT="$ORIGINAL_EVAL_PROMPT"; else unset EVAL_PROMPT; fi
  if [[ -n "$ORIGINAL_MODEL_FAILURE_SET" ]]; then MODEL_FAILURE="$ORIGINAL_MODEL_FAILURE"; else unset MODEL_FAILURE; fi
  if [[ -n "$ORIGINAL_PDE_REPO_ROOT_SET" ]]; then export PDE_REPO_ROOT="$ORIGINAL_PDE_REPO_ROOT"; else unset PDE_REPO_ROOT; fi
  if [[ -n "$ORIGINAL_PYTHONDONTWRITEBYTECODE_SET" ]]; then export PYTHONDONTWRITEBYTECODE="$ORIGINAL_PYTHONDONTWRITEBYTECODE"; else unset PYTHONDONTWRITEBYTECODE; fi
  for provider in "${PROVIDER_VARIABLES[@]}"; do
