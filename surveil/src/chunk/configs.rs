...
fn parse_toml_or_ini_chunks(
    ...
    for (index, line) in lines.iter().enumerate() {
        ...
        if trimmed.starts_with("[[") || trimmed.starts_with(']') {
            return ConfigParseResult::Fallback;
        }
        ...
        let Some(key_path) = split_key_path(key) else {
            return ConfigParseResult::Fallback;
        };
...
mod tests {
    ...
    fn build_chunks_emits_expected_config_chunks() {
        ...
        let cases = [
            ...
            Case {
                name: "comment_only_env_falls_back",
                file_name: ".env",
                content: "# comment\n# still comment\n",
                expected: vec![ExpectedConfig {
                    kind: ChunkKind::GenericFallbackWindow,
            ...
            Case {
                name: "comment_only_yaml_falls_back",
                file_name: "config/settings.yaml",
                content: "# comment\n# still comment\n",
                expected: vec![ExpectedConfig {
                    kind: ChunkKind::GenericFallbackWindow,
                    start_line: 1,
            ...
            Case {
                name: "whitespace_only_json_falls_back",
            ...
            Case {
                name: "empty_json_object_falls_back",
                file_name: "config/settings.json",
                content: "{\n}\n",
                expected: vec![ExpectedConfig {
                    kind: ChunkKind::GenericFallbackWindow,
