...
pub(super) fn is_markdown(path: &Path) -> bool {
    matches!(path.extension().and_then(|ext| ext.to_str()), Some("md" | "markdown" | "mdx"))
}
...
pub(super) fn build_markdown_chunks(source: &SourceFile, text: &str, line_starts: &[usize]) -> Vec<Chunk> {
    ...
    while index < lines.len() {
        ...
        if let Some(fence) = fence_marker(trimmed) {
            ...
            index += 1;
            while index < lines.len() {
                if lines[index].trim_start().starts_with(fence) { index += 1; break; }
                index += 1;
            }
            let end_line = index as u32;
            chunks.push(make_chunk(source, ChunkKind::MarkdownBlock, start_line, end_line, slice_lines(text, line_starts, start_line, end_line), None, None, section_path.clone(), None));
            continue;
        }
        if is_list_line(trimmed) {
            ...
            while index < lines.len() {
                ...
                if next.is_empty() || markdown_heading(next).is_some() || fence_marker(next).is_some() { break; }
                if is_list_line(next) || lines[index].starts_with(' ') || lines[index].starts_with('\t') { index += 1; continue; }
                break;
            }
            let end_line = index as u32;
            chunks.push(make_chunk(source, ChunkKind::MarkdownBlock, start_line, end_line, slice_lines(text, line_starts, start_line, end_line), None, None, section_path.clone(), None));
        ...
        while index < lines.len() {
            let next = lines[index].trim();
            if next.is_empty() || markdown_heading(next).is_some() || fence_marker(next).is_some() || is_list_line(next) { break; }
            index += 1;
        }
        let end_line = index as u32;
        chunks.push(make_chunk(source, ChunkKind::MarkdownBlock, start_line, end_line, slice_lines(text, line_starts, start_line, end_line), None, None, section_path.clone(), None));
...
fn fence_marker(line: &str) -> Option<&'static str> {
    if line.starts_with("```") { Some("```") } else if line.starts_with("~~~") { Some("~~~") } else { None }
}
...
fn is_list_line(line: &str) -> bool {
    line.starts_with("- ")
        || line.starts_with("* ")
        || line.starts_with("+ ")
        || line.split_once('.').is_some_and(|(head, tail)| !head.is_empty() && head.chars().all(|ch| ch.is_ascii_digit()) && tail.starts_with(' '))
}
...
mod tests {
    ...
    fn build_chunks_emits_expected_markdown_chunks() {
        struct ExpectedMarkdown<'a> { start_line: u32, end_line: u32, text: &'a str, section_path: &'a [&'a str] }
        struct Case<'a> { name: &'a str, content: &'a str, expected: Vec<ExpectedMarkdown<'a>> }
        let cases = [
        ...
        for case in cases {
            ...
            let markdown: Vec<_> = chunks.iter().filter(|chunk| chunk.kind == ChunkKind::MarkdownBlock).collect();
            ...
            for (chunk, expected) in markdown.iter().zip(case.expected.iter()) {
                let expected_section_path: Vec<_> = expected.section_path.iter().map(|part| (*part).to_string()).collect();
            ...
            for pair in markdown.windows(2) {
                assert!(pair[0].end_line < pair[1].start_line, "case: {} markdown chunks overlap: {:?} {:?}", case.name, pair[0], pair[1]);
            }
